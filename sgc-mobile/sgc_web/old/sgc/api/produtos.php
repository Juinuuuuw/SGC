...
switch ($method) {
    case 'GET':
        ...
        // Listagem normal de todos os produtos (com JOIN para obter nome do grupo e id_pai)
        $sql = "SELECT p.*, g.id_pai as grupo_pai_id, g.nome as grupo_nome 
                FROM produtos p 
                LEFT JOIN grupos_produtos g ON p.id_grupo = g.id 
    ...
    case 'POST':
        ...
        // Se houver subgrupo selecionado, ele assume a prioridade como id_grupo final
        $id_grupo_enviado = !empty($data['id_grupo']) ? (int)$data['id_grupo'] : null;
        $id_subgrupo = !empty($data['id_subgrupo']) ? (int)$data['id_subgrupo'] : null;
        $id_grupo_final = $id_subgrupo ? $id_subgrupo : $id_grupo_enviado;
        $id_grupo_sql = $id_grupo_final ? $id_grupo_final : 'NULL';

        $referencia = $conn->real_escape_string($data['referencia'] ?? '');
        $nome = $conn->real_escape_string($data['nome']);
        $unidade_venda = $conn->real_escape_string($data['unidadeVenda'] ?? $data['unidade_venda'] ?? 'UN');
        ...
        $sql = "INSERT INTO produtos (id_empresa, referencia, nome, unidade_venda, id_grupo, descricao, preco_custo, margem, preco_venda, estoque)
                VALUES ('$idEmpresa', '$referencia', '$nome', '$unidade_venda', $id_grupo_sql, '$descricao', '$preco_custo', '$margem', '$preco_venda', '$estoque')";
    ...
    case 'PUT':
        ...
        // Mesma lógica de resolução do id_grupo final para edição
        $id_grupo_enviado = !empty($data['id_grupo']) ? (int)$data['id_grupo'] : null;
        $id_subgrupo = !empty($data['id_subgrupo']) ? (int)$data['id_subgrupo'] : null;
        $id_grupo_final = $id_subgrupo ? $id_subgrupo : $id_grupo_enviado;
        $id_grupo_sql = $id_grupo_final ? $id_grupo_final : 'NULL';
        ...
        $sql = "UPDATE produtos SET
                    ...
                    id_grupo=$id_grupo_sql, descricao='$descricao', preco_custo='$preco_custo',
                    margem='$margem', preco_venda='$preco_venda'
